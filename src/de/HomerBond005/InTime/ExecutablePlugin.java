...
public class ExecutablePlugin extends Executable {
	...
	private PluginType pltype;
	private PluginManager pm;
	
	public ExecutablePlugin(String plugin, Set<String> times, Weekday weekday, PluginType pltype){
		super(times, weekday);
		this.plugin = plugin;
	...
	@Override
	public boolean executeIfMatches(int hours, int minutes, Weekday weekday, String args){
		if(this.weekday != Weekday.ALL&&this.weekday != weekday)
			return false;
		for(String time : times){
			String start = time.split("-")[0];
